...
test('logFuelPurchase stores a normalized record', () => {
  const nowRef = { value: 1_700_000_000_000 };
  const logger = makeLogger(nowRef);
  const record = logger.logFuelPurchase('drv_1', { amount: 50, currency: 'gbp', volume: 40, unit: 'liters' });
  ...
  assert.equal(logger.list('drv_1').length, 1);
});

test('logFuelPurchase converts a gallons purchase to liters', () => {
  const nowRef = { value: 1_700_000_000_000 };
  const logger = makeLogger(nowRef);
  const record = logger.logFuelPurchase('drv_1', { amount: 30, currency: 'USD', volume: 10, unit: 'gallon' });
  assert.equal(record.volumeLiters, 37.854);
});

test('logFuelPurchase rejects invalid input', () => {
  const nowRef = { value: 1_700_000_000_000 };
  const logger = makeLogger(nowRef);
  assert.throws(
...
test('logChargingSession stores a normalized record', () => {
  const nowRef = { value: 1_700_000_000_000 };
  const logger = makeLogger(nowRef);
  const record = logger.logChargingSession('drv_1', { cost: 20, currency: 'EUR', kWh: 45 });
  assert.equal(record.type, 'charging');
  assert.equal(record.costBase, 21.6); // 20 * 1.08
  assert.equal(record.kWh, 45);
  assert.equal(logger.list('drv_1', { type: 'charging' }).length, 1);
});

test('list separates fuel purchases from charging sessions', () => {
  const nowRef = { value: 1_700_000_000_000 };
  const logger = makeLogger(nowRef);
  logger.logFuelPurchase('drv_1', { amount: 10, currency: 'USD', volume: 5, unit: 'gallon' });
  logger.logChargingSession('drv_1', { cost: 15, currency: 'USD', kWh: 30 });
  assert.equal(logger.list('drv_1').length, 2);
  assert.equal(logger.list('drv_1', { type: 'fuel' }).length, 1);
  assert.equal(logger.list('drv_1', { type: 'charging' }).length, 1);
});
