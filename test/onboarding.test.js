...
test('a full happy-path run collects entity type and region into a profile', () => {
  ...
  state = wizard.submitStep('usr_1', 'tax_residency', US_DECLARATION);
...
test('choices are matched case-insensitively and normalized to canonical ids', () => {
  ...
  wizard.submitStep('usr_1', 'tax_residency', US_DECLARATION);
  const profile = wizard.complete('usr_1');
...
test('the tax residency step exposes jurisdictions and their tax id types', () => {
  const wizard = makeWizard();
  wizard.start('usr_1');
  wizard.submitStep('usr_1', 'entity_type', 'sole_proprietor');
  const state = wizard.submitStep('usr_1', 'region', 'US-CA');
  assert.equal(state.currentStep.id, 'tax_residency');
...
test('the tax residency step validates the declaration immediately on submit', () => {
  const wizard = makeWizard();
  wizard.start('usr_1');
  wizard.submitStep('usr_1', 'entity_type', 'sole_proprietor');
  wizard.submitStep('usr_1', 'region', 'US-CA');
  ...
  assert.equal(wizard.getState('usr_1').currentStep.id, 'tax_residency');
...
test('a HMRC declaration flows through to the completed profile', () => {
  const wizard = makeWizard();
  wizard.start('usr_1');
  wizard.submitStep('usr_1', 'entity_type', 'sole_proprietor');
  wizard.submitStep('usr_1', 'region', 'US-CA');
  wizard.submitStep('usr_1', 'tax_residency', { jurisdiction: 'UK', taxId: 'AB 12 34 56 C', confirmed: true });
  const profile = wizard.complete('usr_1');
